...
(binary ? describe : describe.skip)('DouyinLiveSidecarManager integration', () => {
  ...
  // Windows SO_REUSEADDR lets the binary share an occupied port, so the
  // bind-conflict rejection only holds on Unix; verified in the Windows POC (M4-05).
  it.skipIf(process.platform === 'win32')('rejects startup when the ws port is already occupied', async () => {
