...
5. 若调用 LLM：已渲染提示词原文、非敏感请求参数、provider/model/request ID、原始响应、解析后的 `quick_reply`/`cue[]`、结构/安全校验结果和错误信息；
...
任务栏/应用图标的唯一设计源为 `svg/douyin-echocue-client-app-icon.svg`；托盘图标的唯一设计源为 `svg/douyin-echocue-client-tray-icon.svg`。Windows 安装包及运行时所需 ICO/PNG 均从上述 SVG 构建生成，不得另行替换图标设计。
...
| 时延 | 采样有效建议的端到端时延以 P95 ≤ 3 秒为持续优化的业务目标；起点为弹幕到达接入服务，终点为 client 完成浮窗展示。未达目标不阻断后续开发，但不得被视为最终可接受结果。 |
...
| O-04 | Markdown 人设/禁忌的本地文件组织、加密、备份与版本策略；DeepSeek API Key 的本地配置与安全存储。 | 技术详细设计 |
