...
> **2026-08 覆盖（有意变更）**：新增「弹幕排队」运行机制（默认关闭）。开启后，展示期间到达的弹幕按 FIFO 排队，展示结束依次补发处理；排队项超过超时（默认 30s）则丢弃并记 `QUEUE_TIMEOUT`。此条款覆盖本 PRD 其他处「绝不排队补发」的表述（对应实现 WP-2，UI 见系统设置→运行机制）。
...
> **2026-08 覆盖（有意变更）**：① 「置信度阈值不暴露」解除——运行页开放 golden 直出阈值（默认 0.85）与语义丢弃阈值（默认 0.9，0–1 可调），下次启动服务生效（WP-4）；② 「绝不排队补发」可按系统设置→运行机制→弹幕排队开关覆盖（WP-2，见场景 D 覆盖说明）。
