-- self-transaction
-- semantic-reason-split: add FILTER_RISK_DISCARD/WINDOW_EVICTED/RETRIEVAL_FAILED/
-- PERSONA_ROUTE_UNAVAILABLE/PIPELINE_ERROR to audit_transition.reason_code CHECK
-- (LOW_VALUE stays for the low_value semantic discard only). SQLite cannot ALTER a
-- CHECK constraint, so the table is rebuilt (same pattern as 002/003).
-- foreign_keys must be OFF for the DROP (audit_reference references
-- audit_transition), which is a no-op inside a transaction — hence the
-- `-- self-transaction` marker: this file owns its BEGIN/COMMIT and pragmas.
...
CREATE TABLE audit_transition_new (
  ...
  reason_code TEXT NOT NULL CHECK (
    reason_code IN (
      ...
      'SOURCE_ERROR','ROOM_ENDED','USER_STOPPED','PIPELINE_ERROR'
    )
  ),
