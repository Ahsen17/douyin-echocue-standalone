-- self-transaction
-- comment-emoji-whitespace-normalize: add 'EMPTY_NORMALIZED' to
-- audit_transition.reason_code CHECK. SQLite cannot ALTER a CHECK constraint,
-- so the table is rebuilt (same pattern as 002). foreign_keys must be OFF for
-- the DROP (audit_reference references audit_transition), which is a no-op
