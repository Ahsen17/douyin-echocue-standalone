...
| `NORMALIZED` | `DISCARDED` | `DISPLAY_WINDOW_ACTIVE`、`EMPTY_NORMALIZED`、`PERSONA_ROUTE_UNAVAILABLE`、兜底/取消/停服类 reason |
| `ROUTED` | `RETRIEVING` | `RETRIEVAL_STARTED` |
| `RETRIEVING` | `DIRECT_READY` | `GOLDEN_DIRECT_ELIGIBLE` |
| `RETRIEVING` | `PROMPT_RENDERED` | `LLM_REQUIRED` |
| `RETRIEVING` | `DISCARDED` | `LOW_VALUE`、`FILTER_RISK_DISCARD`、`RETRIEVAL_FAILED`、`WINDOW_EVICTED`、兜底/取消/停服类 reason |
