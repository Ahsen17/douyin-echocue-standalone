...
界面服务两类用户：出镜人员在直播中只查看浮窗建议；配置人员在主窗口完成启动、配置、审计和打标。MVP 不提供自动回复、云端账号体系、多直播间管理或数据导出。历史建议以**独立历史窗口**承载（见 §5.x），现有浮窗仍只展示当下一条建议、不提供历史列表。

1. 运行页的第一目标是清楚地告知“能否开始监听”和“当前是否正在工作”，而不是展示技术状态细节。
2. 浮窗只承载当下的一条建议；展示窗口内不出现下一条建议，也不提供历史列表。历史回看在独立历史窗口完成。
...
### 5.1 历史建议窗口（gap 任务 history-window）

历史窗口是**独立**置顶无边框浮动面板，与浮窗并列但不承载“当下一条建议”：服务进入运行态时自动显示，按时间顺序追加展示最近 `maxEntries` 条成功展示的建议（每条 = `@昵称`、弹幕正文、短回复建议、2–3 条提词），超过上限自动清理最旧条目。

- 打开方式：服务 RUNNING 自动显示；停止服务后窗口立即隐藏（清空内容，再次运行重新显示），避免空置顶面板遮挡直播；退出程序随进程销毁。
- 数据边界：仅存主进程内存，**不持久化**；停止服务或退出程序自动清空。仅记录成功展示的建议；被过滤/丢弃内容仍只在审计工作区可见。
- 条数上限：默认 20、可配置、上限 120（系统设置「历史窗口」页签）。
- 视觉：复用浮窗主题与字号偏好（`settings.overlay.theme/fontScale`），保证两窗口一致。
- 交互：列表可滚动；新条目到达时仅当用户已处于底部才自动滚到底部，避免打断向上回看。顶部条可拖拽窗口，列表区正常滚动。
- 权限：历史 preload 只读快照与偏好事件，无配置/人设/审计/服务控制；`trace_id` 不进入历史条目。
