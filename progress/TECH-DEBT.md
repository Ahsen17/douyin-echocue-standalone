...
| TD-06 | UI 界面优化 | 运行页/配置页体验待优化，后续排期，有参考页面样式 | 待排期 |
| TD-07 | 人设已发布内容无查看入口 | 发布人设版本后无法查看具体发布了什么，只有发布表单 | 待排期 |
| TD-08 | LLM 提示词无页面化调整 | system prompt 硬编码，无法配置；user 部分涉及数据需留在代码 | 待排期 |
